...
function OpportunityEdit() {
    ...
    useEffect(() => {
        apiGet(`${process.env.REACT_APP_API_HOST}/opportunities/${opportunity_id}`)
        .then(data => {
    ...
    const handleSubmit = (e) => {
        ...
        apiPatch(`${process.env.REACT_APP_API_HOST}/opportunities/${opportunity_id}`, formData)
        .then(data => {
    ...
    const handleDelete = (e) => {
        e.stopPropagation();
        if (!window.confirm(`Delete "${opportunity.title}"? This action cannot be undone.`)) return;
            apiDelete(`${process.env.REACT_APP_API_HOST}/opportunities/${opportunity.id}`)
            .catch((error) => {
